export async function runLabsSelectionUiScenario(page, { timeoutMs, scenario }) {
  const interactionResults = [];

    if (scenario === "labs-select") {
      let step = "init";
      try {
    ...
    } else if (scenario === "labs-select-fitviewport") {
      let step = "init";
      try {
        const trigger = page.locator("#select-trigger-long");
        if (!(await trigger.count())) {
          interactionResults.push({
            name: "labs-select-fitviewport",
            ok: false,
    ...
    } else if (scenario === "labs-select-flip") {
      let step = "init";
      try {
        const trigger = page.locator("#select-trigger-flip");
        if (!(await trigger.count())) {
          interactionResults.push({
            name: "labs-select-flip",
            ok: false,
          ...
          const metrics = await page.evaluate(() => {
            ...
            return {
              vw: window.innerWidth,
              vh: window.innerHeight,
              placement: lb.getAttribute("data-solidus-placement"),
    ...
    } else if (scenario === "labs-select-flip-horizontal") {
      let step = "init";
      try {
        const trigger = page.locator("#select-trigger-flip-h");
        if (!(await trigger.count())) {
          interactionResults.push({
            name: "labs-select-flip-horizontal",
            ok: false,
    ...
    } else if (scenario === "labs-select-slide-overlap") {
      let step = "init";
      try {
        const padding = 8;
        ...
        const openAndRead = async (triggerSel, listboxSel) => {
          ...
          await page.waitForFunction(
            (sel) =>
              document.querySelector(sel)?.getAttribute("data-solidus-placement") !=
              null,
            listboxSel,
            { timeout: timeoutMs },
          );
        ...
        interactionResults.push({
          name: "labs-select-slide-overlap",
          ok,
    ...
    } else if (scenario === "labs-select-clickthrough") {
      let step = "init";
      try {
        const trigger = page.locator("#select-trigger");
    ...
    } else if (scenario === "labs-selection") {
      let step = "init";
      try {
    ...
    } else if (scenario === "labs-combobox") {
      let step = "init";
      try {
    ...
    } else if (scenario === "labs-combobox-arrow-integration") {
      let step = "init";
      try {
        const input = page.locator("#combobox-input-arrow");
        if (!(await input.count())) {
          interactionResults.push({
            name: "labs-combobox-arrow-integration",
            ok: false,
          ...
          const metrics = await page.evaluate(() => {
            ...
            if (!lb) return null;
            const arrow = lb.querySelector("[data-solidus-popper-arrow]");
            const placement = lb.getAttribute("data-solidus-placement") ?? "";
    ...
    } else if (scenario === "labs-combobox-fitviewport") {
      let step = "init";
      try {
        const input = page.locator("#combobox-input");
        if (!(await input.count())) {
          interactionResults.push({
            name: "labs-combobox-fitviewport",
            ok: false,
...
export const labsSelectionUiScenarios = {
  "labs-select": (page, ctx) => runLabsSelectionUiScenario(page, { ...ctx, scenario: "labs-select" }),
  "labs-select-fitviewport": (page, ctx) => runLabsSelectionUiScenario(page, { ...ctx, scenario: "labs-select-fitviewport" }),
  "labs-select-flip": (page, ctx) => runLabsSelectionUiScenario(page, { ...ctx, scenario: "labs-select-flip" }),
  "labs-select-flip-horizontal": (page, ctx) => runLabsSelectionUiScenario(page, { ...ctx, scenario: "labs-select-flip-horizontal" }),
  "labs-select-slide-overlap": (page, ctx) => runLabsSelectionUiScenario(page, { ...ctx, scenario: "labs-select-slide-overlap" }),
  "labs-select-clickthrough": (page, ctx) => runLabsSelectionUiScenario(page, { ...ctx, scenario: "labs-select-clickthrough" }),
  "labs-listbox": (page, ctx) => runLabsSelectionUiScenario(page, { ...ctx, scenario: "labs-listbox" }),
  "labs-selection": (page, ctx) => runLabsSelectionUiScenario(page, { ...ctx, scenario: "labs-selection" }),
  "labs-toast": (page, ctx) => runLabsSelectionUiScenario(page, { ...ctx, scenario: "labs-toast" }),
  "labs-combobox": (page, ctx) => runLabsSelectionUiScenario(page, { ...ctx, scenario: "labs-combobox" }),
  "labs-combobox-arrow-integration": (page, ctx) => runLabsSelectionUiScenario(page, { ...ctx, scenario: "labs-combobox-arrow-integration" }),
  "labs-combobox-fitviewport": (page, ctx) => runLabsSelectionUiScenario(page, { ...ctx, scenario: "labs-combobox-fitviewport" }),
};
