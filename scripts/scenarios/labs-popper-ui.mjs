export async function runLabsPopperUiScenario(page, { timeoutMs, scenario }) {
  const interactionResults = [];

    if (scenario === "labs-popover") {
      try {
    ...
    } else if (scenario === "labs-popover-clickthrough") {
      let step = "init";
      try {
        const trigger = page.locator("#popover-trigger");
        ...
        if (!(await trigger.count()) || !(await outside.count())) {
          interactionResults.push({
            name: "labs-popover-clickthrough",
            ok: false,
    ...
    } else if (scenario === "labs-popover-position") {
      try {
        const trigger = page.locator("#popover-trigger");
        if (!(await trigger.count())) {
          interactionResults.push({
            name: "labs-popover-position",
            ok: false,
            details: { reason: "missing #popover-trigger" },
          });
        } else {
    ...
    } else if (scenario === "labs-popover-flip") {
      try {
        const trigger = page.locator("#popover-trigger-bottom");
        if (!(await trigger.count())) {
          interactionResults.push({
            name: "labs-popover-flip",
            ok: false,
            details: { reason: "missing #popover-trigger-bottom" },
          });
        } else {
          await page.evaluate(() => {
            const el = document.querySelector("#popover-trigger-bottom");
            el?.scrollIntoView?.({ block: "end" });
            // Nudge up so fixed bottom controls don't intercept the click.
            window.scrollBy(0, -260);
          });
          await page.waitForTimeout(150);

          // Fixed bottom controls can overlap the bottom trigger in small viewports.
          await page.evaluate(() => {
            for (const id of [
              "popover-trigger-hide",
              "popover-trigger-arrow",
              "popover-toggle-hide-anchor",
            ]) {
          ...
          await trigger.first().click({ timeout: timeoutMs, position: { x: 5, y: 5 } });
          await page.waitForFunction(
          ...
          await page.waitForFunction(
            () =>
              document
                ...
                ?.getAttribute("data-solidus-placement") != null,
            { timeout: timeoutMs },
          );
          await page.waitForTimeout(80);
          ...
          const metrics = await page.evaluate(() => {
            ...
            return {
              ...
              placement: panel.getAttribute("data-solidus-placement"),
            };
          });
    ...
    } else if (scenario === "labs-popover-flip-horizontal") {
      let step = "init";
      try {
        const trigger = page.locator("#popover-trigger-flip-h");
        if (!(await trigger.count())) {
          interactionResults.push({
            name: "labs-popover-flip-horizontal",
            ok: false,
    ...
    } else if (scenario === "labs-popover-shift") {
      let step = "init";
      try {
        const trigger = page.locator("#popover-trigger-shift");
        if (!(await trigger.count())) {
          interactionResults.push({
            name: "labs-popover-shift",
            ok: false,
    ...
    } else if (scenario === "labs-popover-resize") {
      try {
        const trigger = page.locator("#popover-trigger-edge");
        if (!(await trigger.count())) {
          interactionResults.push({
            name: "labs-popover-resize",
            ok: false,
    ...
    } else if (scenario === "labs-popover-slide-overlap") {
      let step = "init";
      try {
        const padding = 8;
          ...
          const m = await page.evaluate(
            ({ panelSel }) => {
              ...
              return {
                ...
                placement: panel.getAttribute("data-solidus-placement"),
                transform: cs.transform,
                availableWidth: style.getPropertyValue("--solidus-popper-content-available-width") || null,
                availableHeight: style.getPropertyValue("--solidus-popper-content-available-height") || null,
              };
            },
            { panelSel },
          );
        ...
        interactionResults.push({
          name: "labs-popover-slide-overlap",
          ok,
    ...
    } else if (scenario === "labs-popover-hide-detached") {
      let step = "init";
      try {
        const trigger = page.locator("#popover-trigger-hide");
        if (!(await trigger.count())) {
          interactionResults.push({
            name: "labs-popover-hide-detached",
            ok: false,
    ...
    } else if (scenario === "labs-popover-arrow") {
      let step = "init";
      try {
        const trigger = page.locator("#popover-trigger-arrow");
        if (!(await trigger.count())) {
          interactionResults.push({
            name: "labs-popover-arrow",
            ok: false,
          ...
          const metrics = await page.evaluate(() => {
            ...
            const arrow = panel?.querySelector("[data-solidus-popper-arrow]");
            if (!panel || !arrow) return null;
            const pr = panel.getBoundingClientRect();
            const ar = arrow.getBoundingClientRect();
            const placement = panel.getAttribute("data-solidus-placement") ?? "";
    ...
    } else if (scenario === "labs-tooltip-edge") {
      let step = "init";
      try {
        const trigger = page.locator("#tooltip-edge-trigger");
        if (!(await trigger.count())) {
          interactionResults.push({
            name: "labs-tooltip-edge",
            ok: false,
          ...
          const metrics = await page.evaluate(() => {
            ...
            return {
              ...
              placement: p.getAttribute("data-solidus-placement"),
            };
          });
    ...
    } else if (scenario === "labs-tooltip-arrow") {
      let step = "init";
      try {
        const trigger = page.locator("#tooltip-arrow-trigger");
        if (!(await trigger.count())) {
          interactionResults.push({
            name: "labs-tooltip-arrow",
            ok: false,
    ...
    } else if (scenario === "labs-tooltip-slide-overlap") {
      let step = "init";
      try {
        const padding = 8;
        await page.setViewportSize({ width: 420, height: 320 });
        ...
        interactionResults.push({
          name: "labs-tooltip-slide-overlap",
          ok,
          details: { slideOff, slideOn, overlapOff, overlapOn },
        });
      } catch (e) {
...
export const labsPopperUiScenarios = {
  "labs-popover": (page, ctx) => runLabsPopperUiScenario(page, { ...ctx, scenario: "labs-popover" }),
  "labs-popover-clickthrough": (page, ctx) => runLabsPopperUiScenario(page, { ...ctx, scenario: "labs-popover-clickthrough" }),
  "labs-popover-position": (page, ctx) => runLabsPopperUiScenario(page, { ...ctx, scenario: "labs-popover-position" }),
  "labs-popover-flip": (page, ctx) => runLabsPopperUiScenario(page, { ...ctx, scenario: "labs-popover-flip" }),
  "labs-popover-flip-horizontal": (page, ctx) => runLabsPopperUiScenario(page, { ...ctx, scenario: "labs-popover-flip-horizontal" }),
  "labs-popover-shift": (page, ctx) => runLabsPopperUiScenario(page, { ...ctx, scenario: "labs-popover-shift" }),
  "labs-popover-resize": (page, ctx) => runLabsPopperUiScenario(page, { ...ctx, scenario: "labs-popover-resize" }),
  "labs-popover-slide-overlap": (page, ctx) => runLabsPopperUiScenario(page, { ...ctx, scenario: "labs-popover-slide-overlap" }),
  "labs-popover-hide-detached": (page, ctx) => runLabsPopperUiScenario(page, { ...ctx, scenario: "labs-popover-hide-detached" }),
  "labs-popover-arrow": (page, ctx) => runLabsPopperUiScenario(page, { ...ctx, scenario: "labs-popover-arrow" }),
  "labs-tooltip": (page, ctx) => runLabsPopperUiScenario(page, { ...ctx, scenario: "labs-tooltip" }),
  "labs-tooltip-edge": (page, ctx) => runLabsPopperUiScenario(page, { ...ctx, scenario: "labs-tooltip-edge" }),
  "labs-tooltip-arrow": (page, ctx) => runLabsPopperUiScenario(page, { ...ctx, scenario: "labs-tooltip-arrow" }),
  "labs-tooltip-slide-overlap": (page, ctx) => runLabsPopperUiScenario(page, { ...ctx, scenario: "labs-tooltip-slide-overlap" }),
};
