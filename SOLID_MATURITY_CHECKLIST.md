# Solid (Dart DOM) Maturity Checklist for a 3‑Panel Word Processor

This repo’s Solid-style runtime (`lib/solid/` + `lib/solid_dom/`) is already sufficient for building the **application shell** (layout, navigation, panels, overlays, menus, forms, keyboard UX). The biggest product risk (rich-text editing) is intentionally delegated to a JS editor via interop.
...
- Solid primitives scenarios:
  ...
  - `npm run debug:solid-wordproc` (3-panel stress shell)
